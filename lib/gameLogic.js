...
// This code updates the processAttack function in gameLogic.js
// to limit enemy stun duration to a maximum of 2 rounds
...
const processAttack = (gameState, ability) => {
     ...
     if (ability.statusEffect) {
          // IMPORTANT FIX: Limit stun duration to 2 turns maximum for enemies
          let effectToApply = { ...ability.statusEffect };

          // If this is a stun effect, cap the duration to 2 turns
          if (effectToApply.type === 'stun') {
               effectToApply.duration = Math.min(effectToApply.duration, 2);
               console.log(`Stun effect duration limited to ${effectToApply.duration} turns`);
          }

          // Add the status effect with limited duration
     ...
     return {
          ...gameState,
          logMessage: `${critText}${ability.name} deals ${finalDamage} damage to ${gameState.currentEnemy.name}!${ability.statusEffect?.type === 'stun' ? " Enemy STUNNED!" : ""
               }`
     };
};
...
const processEffectResolution = (gameState) => {
     ...
     if (newState.phase === GAME_CONSTANTS.GAME_PHASES.BATTLE) {
          ...
          // FIX BEGINS HERE: Update cooldowns for Aurora's abilities
          // Make sure the cooldowns object exists
          if (!newState.aurora.cooldowns) {
               newState.aurora.cooldowns = {};
          }
          
          // Loop through all cooldowns and decrease them by 1 if they're above 0
          Object.keys(newState.aurora.cooldowns).forEach(abilityName => {
               if (newState.aurora.cooldowns[abilityName] > 0) {
                    ...
                    // Log cooldown reduction for debugging
                    effectMessages.push(`${abilityName} cooldown reduced to ${newState.aurora.cooldowns[abilityName]}`);
               }
          });
     ...
     // Ensure turn phase is properly set
     newState.turnPhase = GAME_CONSTANTS.TURN_PHASES.PLAYER1;
